...
*/
    ...
    Ext.define('canvasteroids.Game', {
        ...
        requires: ['canvasutils.CanvasUtils', 'canvasteroids.Rock', 'canvasteroids.Ship', 'canvasteroids.ShipFragment', 'canvasteroids.Bullet', 'renderer.CanvasRenderer', 'geometry.Plane', 'controller.Keyboard'],
        ...
        recycleBullet: function (bullet_props) {
            var bullet;
            for (var b = 0; b < this.bullets.length; b++) {
                bullet = this.bullets[b];
                if (bullet.active === false) {
                    Ext.apply(bullet, bullet_props);
                    bullet.active = true;
                    return true;
                }
            }
        ...
        fireBullet: function (bullet_props) {
            var bullet;
            if (!this.recycleBullet(bullet_props)) {
                bullet = new canvasteroids.Bullet(bullet_props);
...
/*animateRocks: function () {
        ...
        recycleRock: function (rock_props) {
            var rock;
            for (var r = 0; r < this.rocks.length; r++) {
                rock = this.rocks[r];
                if (rock.active === false) {
                    Ext.apply(rock, rock_props);
                    rock.init(rock_props);
                    rock.active = true;
                    this.plane.addShape(rock);
                    return true;
                }
        ...
        //from the array, rather than using an object pool, as was done for the bullets
        explodeRock: function (rock) {
            var newRock, rock_conf;
            //deactivate and remove rock from plane
            //only create new rocks if it is not the smallest
            if (rock.size > 1) {
                for (var i = 0; i < 3; i++) {
                    rock_conf = {
                        context: this,
                    ...
                    //try to recycle a o rock
                    if (!this.recycleRock(rock_conf)) {
                        newRock = new canvasteroids.Rock(rock_conf);
        ...
        START_LIFE: function (msg) {
            ...
            case 'enter':
                //TODO: don't make a new ship every time
