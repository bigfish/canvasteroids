...
define(function (require) {
    ...
    //require utilities
    //polyfill isPointInPath function
    //function (ctx, x, y) => Boolean
    var isPointInPath = require('canvas-utils/isPointInPath');

    //function (ctx, poly1, poly2) ==> Boolean
    var polygonsIntersect = require("canvas-utils/polygonsIntersect");
...
*/
    ...
    Rock.prototype.hit = function () {
        ...
        for (var i = 0; i < bullets.length; i++) {
            ...
            if (isPointInPath(ctx, bullet.x, bullet.y, this.x, this.y)) {
                bullet.active = false;
                return true;
            }
        }
    ...
    function makeRocks() {
        ...
        for (var r = 0; r < num_rocks; r++) {
            ...
            //do not clobber ship
            if (ship && state === START_LEVEL && polygonsIntersect(ctx, rock, ship)) {
                num_rocks++;
                continue;
            }
            rocks.push(rock);
    ...
    function hitRock(ship, rock) {
        ...
        if (Math.abs(ship.x - rock.x) > range || Math.abs(ship.y - rock.y) > range) {
            ...
            return polygonsIntersect(ctx, rock, ship);
        }
    }
