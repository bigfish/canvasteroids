/*global canvasutils canvasteroids controller sprites geometry renderer eventbus drawable */
...
*/
    ...
    Ext.define('canvasteroids.Game', {
        ...
        //TODO: extend base Game
        requires: ['eventbus.EventBus', 'canvasutils.CanvasUtils', 'sprites.Rock', 'sprites.Ship', 'sprites.ShipFragment', 'sprites.Bullet', 'drawable.Layer', 'controller.Keyboard'],
        ...
        constructor: function (props) {
            ...
            this.gameLayer = new drawable.Layer({
                x: 0,
                y: 0,
                fullscreen: true,
        ...
        drawStartButton: function () {
            ...
            var btn_width = 100;
            var btn_height = 40;
            var ctx = this.gameLayer.ctx;
            var m = ctx.measureText('PLAY'); //gets width only... height is font size
            //save the current state so we can restore it later
            ctx.save();
            //draw text
            ctx.beginPath();
            ctx.translate(this.gameLayer.canvas_width / 2 - btn_width / 2, this.gameLayer.canvas_height / 2 - btn_height / 2);
            ctx.textAlign = "left";
        ...
        hitRock: function (ship, rock) {
            ...
            if (Math.abs(ship.x - rock.x) > range || Math.abs(ship.y - rock.y) > range) {
                ...
                return polygonsIntersect(this.gameLayer.ctx, rock, ship);
            }
        },
        ...
        START_GAME: function (msg) {
            ...
            function onClick(event) {
                var x = event.clientX - this.gameLayer.canvas.offsetLeft;
                var y = event.clientY - this.gameLayer.canvas.offsetTop;

                if (me.state === me.START_GAME) {
                    me.drawStartButton();
                    if (this.gameLayer.ctx.isPointInPath(x, y)) { //needs fixing after Layer refactor
            ...
            case 'enter':
                this.makeRocks();
                this.startTimer();
                this.gameLayer.canvas.addEventListener('click', Ext.Function.bind(onClick, this), false);
                break;
            ...
            case 'exit':
                this.gameLayer.canvas.removeEventListener('click', Ext.Function.bind(onClick, this), false);
                break;
